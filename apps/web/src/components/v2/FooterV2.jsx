    ...
    <footer className="relative bg-black border-t border-white/8 overflow-hidden">
      ...
      <div className="relative px-10 lg:px-16 pt-16 lg:pt-24 pb-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-12 lg:gap-8 mb-16">
          <div className="lg:col-span-1">
            <a href="/v2" className="flex items-center gap-3 mb-5">
              <img src="/logo.png" alt="ALC" className="h-7 w-7 object-contain brightness-0 invert" onError={(e) => { e.target.style.display = 'none'; }} />
            ...
            <a href="#visit" className="inline-block font-heading font-bold text-xs tracking-[0.3em] uppercase text-white/50 hover:text-gold transition-colors group">
              <span className="text-gold/40 group-hover:text-gold">( </span>Plan Your Visit<span className="text-gold/40 group-hover:text-gold"> )</span>
            </a>
          </div>

          {Object.entries(links).map(([heading, items]) => (
            <div key={heading}>
              <p className="text-white/20 text-[10px] tracking-[0.4em] uppercase mb-4 font-medium">{heading}</p>
              <ul className="space-y-2">
                {items.map((item) => (
                  <li key={item}>
                    <a href="#" className="text-white/40 hover:text-gold text-xs tracking-[0.15em] uppercase transition-colors">{item}</a>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="border-t border-white/8 pt-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <p className="text-white/20 text-[10px] tracking-[0.25em] uppercase">© {year} Abundant Life Church — Rock Hill, SC</p>
          <p className="text-white/15 text-[10px] tracking-[0.25em] uppercase">abundantliferockhill.org</p>
        </div>
      </div>
    </footer>
  );
