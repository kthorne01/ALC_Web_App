    ...
    <section className="relative py-24 lg:py-36 px-10 lg:px-16 bg-transparent overflow-hidden">
      ...
      <div className="relative max-w-5xl">
        <p className="text-white/30 text-[11px] tracking-[0.4em] uppercase mb-6 font-medium">
          <span className="text-gold/50">[ </span>Why We Exist<span className="text-gold/50"> ]</span>
        </p>

        <h2 className="font-heading font-bold uppercase relative z-10 mb-12" style={{ lineHeight: 0.9, letterSpacing: '-0.02em' }}>
          <span className="block text-white/40" style={{ fontSize: 'clamp(28px, 4vw, 56px)' }}>ALC EXISTS TO</span>
          <span className="block text-white"    style={{ fontSize: 'clamp(48px, 7.5vw, 120px)' }}>HELP PEOPLE</span>
          <span className="block text-gold"     style={{ fontSize: 'clamp(48px, 7.5vw, 120px)' }}>LIVE FULLY.</span>
        </h2>

        <div className="border-t border-white/10">
          {pillars.map((p) => (
            <div key={p.num} className="flex items-center gap-6 py-4 border-b border-white/8 group">
              <span className="font-heading text-white/20 text-xs tracking-[0.3em] w-8 flex-shrink-0">{p.num}</span>
              <span className="font-heading font-bold text-white/70 group-hover:text-gold transition-colors duration-200 tracking-[0.15em] uppercase" style={{ fontSize: 'clamp(18px, 2.5vw, 32px)' }}>
                {p.label}
              </span>
