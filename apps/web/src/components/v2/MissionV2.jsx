    ...
    <section className="relative py-24 lg:py-36 px-10 lg:px-16 bg-transparent overflow-hidden">
      ...
      <div className="relative max-w-5xl">
        ...
        <h2 className="font-heading font-bold uppercase relative z-20 mb-12" style={{ lineHeight: 0.9, letterSpacing: '-0.02em' }}>
          <span className="block text-black/65" style={{ fontSize: 'clamp(28px, 4vw, 56px)' }}>ALC EXISTS TO</span>
        ...
        <div className="border-t border-black/10">
          {pillars.map((p) => (
            <div key={p.num} className="flex items-center gap-6 py-4 border-b border-black/8 group">
              <span className="font-heading text-black/45 text-xs tracking-[0.3em] w-8 flex-shrink-0">{p.num}</span>
              ...
              <span className="ml-auto text-black/45 group-hover:text-gold/50 transition-colors text-xs">→</span>
            </div>
          ))}
        </div>
      </div>
