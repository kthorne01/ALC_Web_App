...
const HeroV2 = () => {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      if (photo1Ref.current) photo1Ref.current.style.transform = `translateY(${y * -0.12}px)`;
      if (photo2Ref.current) photo2Ref.current.style.transform = `translateY(${y * -0.2}px)`;
      if (sermonRef.current) sermonRef.current.style.transform = `translateY(${y * -0.07}px)`;
    };
    ...
    <section className="relative min-h-screen flex flex-col bg-transparent" id="hero">
      ...
      <div className="flex-1 flex flex-col lg:flex-row pt-20 relative">
        ...
        <div className="flex-1 relative flex flex-col justify-center px-10 lg:px-16 py-12 lg:py-20 overflow-visible">
          ...
          >
            <span className="block text-white" style={{ fontSize: 'clamp(72px, 14vw, 210px)' }}>ABUNDANT</span>
            <span className="block text-gold"  style={{ fontSize: 'clamp(72px, 14vw, 210px)' }}>LIFE</span>
            <span className="block text-white" style={{ fontSize: 'clamp(72px, 14vw, 210px)' }}>CHURCH</span>
          </h1>

          <div ref={photo1Ref} className="absolute z-20 hidden lg:block" style={{ top: '32%', left: '4%', width: '22%' }}>
            <PhotoSlot src="/images/hero/hero-worship.jpg" style={{ aspectRatio: '4/5' }} />
          </div>

          <div ref={photo2Ref} className="absolute z-20 hidden lg:block" style={{ bottom: '18%', left: '38%', width: '18%' }}>
        ...
        <div className="lg:w-64 xl:w-72 flex flex-col justify-end px-10 lg:px-8 pb-10 lg:pb-16 pt-0 lg:pt-24 gap-4">
          <p className="text-gold text-[10px] tracking-[0.55em] uppercase font-medium">Latest Sermon</p>
          <a href="#sermons" className="text-white/60 hover:text-gold text-xs tracking-[0.15em] uppercase transition-colors flex items-center gap-2 group">
            <span className="w-5 h-5 rounded-full border border-current flex items-center justify-center text-[8px] flex-shrink-0">▶</span>
            <span>
              <span className="text-gold/60">( </span>WATCH NOW ↗<span className="text-gold/60"> )</span>
            </span>
          </a>
          <div ref={sermonRef}>
      ...
      <div className="border-t border-white/8 py-5 px-10 lg:px-16 flex flex-col sm:flex-row items-start sm:items-center gap-5 sm:gap-10">
        {ctaLinks.map((cta) => (
          <a key={cta.label} href={cta.href} className="text-white/60 hover:text-gold text-xs tracking-[0.2em] uppercase transition-colors duration-200 font-medium group">
